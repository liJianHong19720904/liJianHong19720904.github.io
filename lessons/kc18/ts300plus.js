/****
  本文件ts300plus.js,可以使用script元素导入Web文档。
  导入后，Web文档将产生1个全局变量，ts300。
  ts300的数据类型为数组，数组的元素共309个。
  ts300数组的每个元素都是对象数据类型，
  ts300的每个元素都包含一首唐诗的信息，元素对象数据结构为：
       { 
		title : '' ,
		writer: '' ,
		content: [],
...
*****/
 ...
 ( function(){
	...
	for (i = BEGIN ; i < END  ; i++ ){
        p1 = ts300String.search(toString(i)) ;
		p2 = ts300String.search(toString(i+1)) ;
		if (p1 !== -1 && p2 !== -1){
         ts[i-BEGIN] = ts300String.substring(p1,p2);
		 } 
	}//end of for loop
	ts[ts.length] =ts300String.substring(p2);//最后一首
	...
	//以二个处理好的数组数据 tsTitle和 tsContent为基础，创建更加方便使用的对象数组，数据结构如下代码所示。
     ...
     for (i=0; i < tsTitle.length ; i++ ){
		 ...
		 tsArr.push(ts) ;
		 ts = {} ; 
		 //缺乏这一句，按原始类型的逻辑，似乎可行，但ts是对象类型，这一句定要存在，让ts变量指向一个全新的对象。

     }//创建对象数组循环结束
	 //代码写到此处，我们的数据只需要tsArr，其他变量可以全部销毁。
    tsTitle = null ;
    tsContent = null ;
    ts300 = null ;
	ts = null ;
	//下面写一段代码，遍历每首唐诗的content，判断是5言诗或者7言诗，存放在新的属性letters中。
     for ( i=0 ; i < tsArr.length ; i++){
	  ...
	  if ( ! tsArr[i].letters )	 { //担心最后一行有拼凑字，造成未能生成letters属性，这是我们可以通过letters属性判读，则使用倒数第二行再次判读。
	...
	//下面写一段代码，遍历每首唐诗的content，将序言从content中提出来，存放在新的属性preface中，这段代码基于唐诗已经计算好的letters属性，解决序的分析判读问题就更接近了。
   ...
   for ( i=0 ; i < tsArr.length ; i++){
		ts = tsArr[i] ;
        if (ts.preface){
			console.log('No:'+(i+1)+'含序。');
        }
   }//在分析序的条件还未完善之时，可以由上面输出结果发现：
   //少数唐诗行文太自由，因此代码没有分析古诗的能力，
   //误判了第 38 、48 、207首。而第55首实际有序言，但由于有拼凑字标识干扰，代码未能正常判别。
